...
CANVI: compta una **finestra diària** per a E/F amb aquesta regla:
  • Dies laborables (no dilluns): només **ahir** [00:00, 24:00)
  • **Dilluns**: des de **divendres 00:00** fins a **dilluns 00:00** (inclou cap de setmana)

Les capçaleres de la pestanya es mantenen com abans per compatibilitat:
  E = "Abiertos Última Semana"  (ara: finestra diària)
  F = "Cerrados Última Semana"  (ara: finestra diària)

Columnes A–N que s'escriuen:
 A. Fecha (DD/MM/YYYY)
 B. AGENCIA
 C. Tickets Abiertos (total a Home)
 D. Tickets Cerrados (total a List Closed)
 E. Abiertos Última Semana (finestra diària actual)
 F. Cerrados Última Semana (finestra diària actual)
 G. DS (ocurrències)
 H. P3 (ocurrències)
 I. Total (= C + D)
 J. LW total (= E + F)
 K. =IFNA(D[actual]-D[actual-22];0)
 L. Semana (YYYY-SS, ISO)
 M. Mes (YYYY-MM)
 N. Año (YYYY)
...
DAILY_ROW_STRIDE=22           # files entre dies consecutius (per defecte 22)
...
# ---------- Finestra temporal diària ----------

def daily_window():
    """Retorna (start, end) naive per comptar la finestra diària.
    — Dilluns: divendres 00:00 → dilluns 00:00
    — Altres dies: ahir 00:00 → avui 00:00
    """
    today = datetime.now().date()  # segons TZ del host
...
def parse_date_any(s: str) -> Optional[datetime]:
    ...
    try: return dtparse.parse(s, dayfirst=False, fuzzy=True)
    except Exception:
...
# ---------- Comptes (Home i Closed) sobre la finestra diària ----------

async def read_home_metrics(page: Page):
    try:
    ...
    for r in rows:
        ...
        if re.search(r'(?i)\bDS\s*-?\s*\d+', target): bugs += 1
        if re.search(r'(?i)\bP\s*3(\b|\.)', target): p3 += 1
        d=parse_date_any(r.get('date',''))
        if d and start <= d < end: win_open += 1
    return {"open_calls": open_calls, "bugs": bugs, "p3": p3, "win_open": win_open}

async def read_closed_window(page: Page, base_url: str):
    try:
    ...
    if await click_last_page(page):
        for _ in range(200):
            ...
            if dmax is not None and dmax < start: break  # pàgina massa antiga
            for r in rows:
                d=parse_date_any(r.get('date',''))
                if d and start <= d < end: total += 1
            if not await click_prev_page(page): break
    else:
        for _ in range(200):
            rows = await extract_table_any(page)
            if not rows: break
...
def append_daily_rows_to_sheet(rows: List[Dict[str,object]]):
    ...
    stride = int(os.getenv("DAILY_ROW_STRIDE", "22"))  # files entre dies consecutius (per defecte 22)
    ...
    for i, r in enumerate(rows):
        rownum = start_row + i
        agencia = r.get('Nombre Agencia','')
        C = _safe_int(r.get('Tickets Abiertos', 0))
        D = _safe_int(r.get('Tickets Cerrados', 0))
        E = _safe_int(r.get('Abiertos Última Semana', 0))
        F = _safe_int(r.get('Cerrados Última Semana', 0))
        G = _safe_int(r.get('DS', 0))
        H = _safe_int(r.get('P3', 0))

        # --- Parxe: si "Cerrados" surt 0, prova a recuperar el valor de fa N files (mateixa agència) ---
        if D == 0 and rownum - stride >= 2:  # hi ha capçalera a la fila 1
            try:
                prev_ag = ws.cell(rownum - stride, 2).value  # Columna B: AGENCIA
                if (prev_ag or '').strip() == (agencia or '').strip():
                    prev_closed = ws.cell(rownum - stride, 4).value  # Columna D: Tickets Cerrados
                    prev_closed_i = _safe_int(prev_closed)
                    if prev_closed_i > 0:
                        print(f"[~] '{agencia}': Cerrados=0 → copio valor anterior (fila {rownum - stride}): {prev_closed_i}")
                        D = prev_closed_i
            except Exception as e:
                print(f"[i] No s'ha pogut llegir la fila anterior per '{agencia}': {e}")

        total_formula = f"=C{rownum}+D{rownum}"
        lw_formula    = f"=E{rownum}+F{rownum}"
        # Nota: la capçalera diu Δ Cerrados(-22d); el salt real es parametritza amb DAILY_ROW_STRIDE
        delta22       = f"=IFNA(D{rownum}-D{rownum - stride};0)" if rownum > stride else "0"
...
async def amain():
    ...
    async with async_playwright() as p:
        ...
        for a in ags:
            ...
            try:
                ...
                home = await read_home_metrics(page)
                closed = await read_closed_window(page, base_url)
                results.append({
    ...
    append_daily_rows_to_sheet(results)
